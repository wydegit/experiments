...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        tbar.set_description('Epoch: %d || Training loss: %.4f' % (epoch, train_loss / len(self.train_data)))
    ...
    def validation(self, epoch):
        ...
        tbar.set_description('Epoch %d || loss: %.4f || pixAcc: %.4f || IoU: %.4f || nIoU: %.4f'
                             % (epoch, val_loss/ len(self.eval_data), pixAcc, mIoU, nIoU))
        val_logger.info("Epoch{:d}  Validation loss:{:.4f}  pixAcc:{:.4f}  mIoU:{:.4f}  nIoU:{:.4f}"
                        .format(epoch, val_loss / len(self.eval_data), pixAcc, mIoU, nIoU))
